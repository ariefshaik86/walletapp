...
$link = "https://walletapp.rf.gd/register.php?ref=" . urlencode($referralCode);

// Fetch referrals
$stmt = $pdo->prepare("SELECT email  FROM users WHERE referred_by = ?");
...
<div id="dashboardContent">
  ...
  <h3>🎁 Refer & Earn</h3>
  <p>Share your unique referral link and earn 10/- when your friends sign up!</p>

  <div class="referral-box">
    <input type="text" id="referralLink" value="<?= htmlspecialchars($link) ?>" readonly>
    ...
    <?php if (count($referrals) > 0): ?>
      <ul>
        <?php foreach ($referrals as $ref): ?>
          <li><?= htmlspecialchars($ref['email']) ?> — Joined on <?= date("d M Y", strtotime($ref['created_at'])) ?></li>
        <?php endforeach; ?>
      </ul>
    <?php else: ?>
      <p style="color: #777;">You haven’t referred anyone yet.</p>
    <?php endif; ?>
  </div>
</div>
