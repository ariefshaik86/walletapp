...
function timeAgo($datetime) {
    date_default_timezone_set('Asia/Kolkata'); // Optional: set your timezone
    ...
    if ($diff->y > 0) {
        ...
        return $diff->i . ' minute' . ($diff->i > 1 ? 's' : '') .
               ($diff->s > 0 ? ', ' . $diff->s . ' second' . ($diff->s > 1 ? 's' : '') : '') . ' ago';
    } elseif ($diff->s > 0) {
...
$completedStmt->execute(['email' => $email]);
$comApprovals = $completedStmt->fetchAll();
    ...
    <main class="main-content">
      ...
      <section class="recent-activity">
       ...
       foreach ($completedApprovals as $task):
          ?>
          <?php $dattime = timeAgo($task['submitted_at']); ?>
          <div class="activity-item">
            ...
            <div class="activity-details">
              ...
              <p>You completed "<?= htmlspecialchars($task['task_title'])?>" task</p>
               <span class="activity-time"><small><?= $dattime ?></small></span>
            </div>
        ...
        endforeach;
            ?>
            <?php
       foreach ($comApprovals as $task):
          ?>
          <?php $datetime = timeAgo($task['requested_at']); ?>
          <div class="activity-item">
            <div class="activity-icon success">
              <i class="fas fa-check"></i>
            </div>
            <div class="activity-details">
              ...
              <p>You have Withdrawn "<?= htmlspecialchars($task['amount'])?>" via "<?= htmlspecialchars($task['method'])?>"</p>
              <span class="activity-time"><small><?= $datetime ?></small></span>
            </div>
            <div class="activity-amount w">- ₹<?= htmlspecialchars($task['amount'])?></div>
          </div>
          <?php
        endforeach;
            ?>
