...
; // prints: arief@example.com


// Fetch pending approvals
$pendingStmt = $pdo->prepare("SELECT * FROM withdrawals WHERE email=:email AND status='pending'");
$pendingStmt->execute(['email' => $email]);
$pendingApprovals = $pendingStmt->fetchAll();

// Fetch completed approvals
$completedStmt =  $pdo->prepare("SELECT * FROM withdrawals WHERE email=:email AND status='approved'");
$completedStmt->execute(['email' => $email]);
$completedApprovals = $completedStmt->fetchAll();

$rejectedStmt =  $pdo->prepare("SELECT * FROM withdrawals WHERE email=:email AND status='rejected'");
...
$rejectedApprovals = $rejectedStmt->fetchAll();
  ...
  $hasAccount = !empty($user['withdrawal_method']);
  ?>
...
<head>
  ...
  animation: fadeInUp 0.5s ease;
    ...
    @media (max-width: 768px) {
      ...
      /* 👇 Map each column header label to its data cell */
      ...
      td:nth-of-type(5)::before { content: "Submitted At"; }
    }
      /* Base Styles */
:root {
  --primary: #4a6bff;
...
/* Wallet Card */
.wallet-balance-card {
  background: linear-gradient(135deg, #4a6bff,rgb(179, 187, 212));
  padding: 20px;
  border-radius: 15px;
  text-align: center;
  margin-bottom: 20px;
...
<body>
  <div class="app-container">
    ...
    <aside class="sidebar">
      ...
      <div class="user-profile">
        ...
        <div class="user-info">
          ...
          <span class="user-email" id="userEmail"><?=htmlspecialchars($user['email'])?></span>
        </div>
      </div>
    </aside>
    ...
    <main class="main-content">
      ...
      <header>
        <div class="header-content">
            <a href="index.php" class="logo">WalletApp</a>
    ...
    <p class="balance-amount">₹<?= htmlspecialchars($wallet) ?>.00</p>
  </div>
  ...
  <form method="POST" action="withdraw_process.php" class="withdraw-form">
    <?php if (!$hasAccount): ?>
      ...
      <div class="method-toggle">
        <button type="button" class="toggle-btn active" onclick="selectMethod('bank')">Bank</button>
        <button type="button" class="toggle-btn" onclick="selectMethod('upi')">UPI</button>
      </div>
      ...
      <div id="upi-fields" class="method-fields hidden">
        <input name="upi_id" placeholder="Your UPI ID (e.g. name@bank)"  />
      </div>

      <button type="submit" class="submit-btn">💾 Save Account Info</button>

    <?php else: ?>
      <h3 class="section-title">Withdraw Funds</h3>
      <div class="account-summary">
        <p><strong>Method:</strong> <?= strtoupper($user['withdrawal_method']) ?></p>
        <?php if ($user['withdrawal_method'] === 'upi'): ?>
          <p>UPI ID: <?= htmlspecialchars($user['upi_id']) ?></p>
        <?php else: ?>
          <p>Bank: <?= htmlspecialchars($user['bank_name']) ?> (<?= htmlspecialchars($user['account_number']) ?>)</p>
        <?php endif; ?>
      </div>

      <input type="hidden" name="method" value="<?= $user['withdrawal_method'] ?>">
      <input id="amount"type="number" name="amount" placeholder="Enter Amount to Withdraw" min="1" max="<?= (int)$wallet ?>" required>
  ...
  <div class="section">
    ...
    <?php if ($completedApprovals): ?>
      ...
      <table>
        ...
        <tbody>
          <?php foreach ($completedApprovals as $row): ?>
            <tr>
              <td><?= htmlspecialchars($row['id']) ?></td>
              <td><?= htmlspecialchars($row['method']) ?></td>
              <td><?= htmlspecialchars($row['amount']) ?></td>
              <td><span class="badge completed"><?= ucfirst($row['status']) ?></span></td>
              <td><?= htmlspecialchars($row['requested_at']) ?></td>
            </tr>
          <?php endforeach; ?>
        </tbody>
  ...
  <div class="section">
    ...
    <?php if ($rejectedApprovals): ?>
      ...
      <table>
        ...
        <tbody>
          <?php foreach ($rejectedApprovals as $row): ?>
            <tr>
              <td><?= htmlspecialchars($row['id']) ?></td>
              <td><?= htmlspecialchars($row['method']) ?></td>
              <td><?= htmlspecialchars($row['amount']) ?></td>
              <td><span class="badge reject"><?= ucfirst($row['status']) ?></span></td>
              <td><?= htmlspecialchars($row['requested_at']) ?></td>
            </tr>
          <?php endforeach; ?>
        </tbody>
